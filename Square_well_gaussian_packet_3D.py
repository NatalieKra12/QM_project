...
def plot_probability(i, Z, line):
    ...
    ax.set_ylabel(r'$y$')
    ax.set_zlabel(r'$z$')
    cs = ax.plot_surface(xp, yp,  np.array(sum_PSI(i/100)), cmap = cm.twilight, linewidth = 0.5)
    # if cbaractive:
...
if __name__ == "__main__":
    ...
    Z = sum_PSI(0)
    Z = np.array(Z)
    line = ax.plot_surface(xp, yp, Z, cmap = cm.twilight, linewidth = 0.5)
    ani = animation.FuncAnimation(fig, plot_probability, fargs = (Z, line), frames = 600, interval=20, blit=False)
    writer = PillowWriter(fps=60)
    ani.save("gauss3D3.gif", writer=writer)
    # pyplot.show()
